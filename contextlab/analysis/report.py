...
def generate_report(root: Path, *, charts: bool = True) -> Path:
    ...
    lines = [
        ...
        "Mock results validate orchestration and must not be interpreted as model performance."
        if config.get("mock")
        else "Results reflect the configured inference endpoint and hardware.",
        "",
        "| Task | Budget | Policy | n | Success | Failed | Success rate | "
        "Median tokens | Median latency (s) | Median tool calls |",
        "|---|---|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            ...
            "{total_tokens_median:.1f} | {wall_seconds_median:.3f} | "
            "{tool_calls_median:.1f} |".format(**row)
        )
    lines.extend(
        [
            "",
            ...
            "",
            "These tables are observations from collected run artifacts. A shorter prompt "
            "can still produce higher total workload when it causes extra model requests, "
            "retrieval, compaction, or repeated repository exploration. Causal claims "
            "require repeated real-model trials and uncertainty analysis; ContextLab does "
            "not infer causality from a repeated tool call alone.",
            "",
            "Successful and failed runs remain separate in `aggregate.json`; token medians for "
            "successful runs are reported independently when available.",
        ]
    )
    report.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return report
...
def _generate_charts(
    ...
    def average(key: str, nested: str | None = None) -> dict[str, float]:
        ...
        for policy in policies:
            ...
            values = [float(item[nested][key] if nested else item[key]) for item in selected]
    ...
    bar(
        ...
        {
            policy: sum(
                float(row["success_rate"]) * int(row["sample_size"])
                for row in rows
    ...
    bar(
        "total-inference-tokens.png",
        "Total inference workload",
        "tokens",
        average("total_tokens", "inference"),
    )
    ...
    bar(
        "retrieval-overhead.png",
        "Retrieved context volume",
        "tokens",
        average("retrieval_tokens", "metrics"),
    )
    bar(
        "compaction-overhead.png",
        "Compaction inference input",
        "tokens",
        average("compaction_input_tokens", "metrics"),
    )
    figure, axis = plt.subplots(figsize=(8, 4.5))
    for item in detailed:
        ...
        if growth:
            ...
            axis.plot(
                ...
                [value for _, value in steps],
                alpha=0.6,
                label=item["policy_id"],
            )
